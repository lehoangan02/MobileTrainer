...
[InitializeOnLoad]
public static class DeploySceneSetup
{
    private const string DeployScenePath = "Assets/Screens/DeployForFlightScreen/DeployForFlight.unity";
    ...
    private static void AutoCheck()
    {
        ...
        if (activeScene.path == DeployScenePath)
        {
            if (GameObject.Find("DeployTutorialCanvas") == null || GameObject.Find("TutorialController") == null)
            {
                ExecuteDeploySceneSetup();
            }
        }
    }

    [MenuItem("Tools/Setup Deploy For Flight Scene")]
    public static void ExecuteDeploySceneSetup()
    {
        Scene activeScene = SceneManager.GetActiveScene();
        if (activeScene.path != DeployScenePath)
        {
            ...
            {
                EditorSceneManager.OpenScene(DeployScenePath);
            }
            else
            {
                Debug.LogWarning("[DeploySceneSetup] Open cancelled.");
                return;
            }
        }

        Debug.Log("[DeploySceneSetup] Configuring Deploy For Flight scene...");

        // 1. Locate TutorialRigRoot
        GameObject rigRootGo = GameObject.Find("TutorialRigRoot");
        if (rigRootGo == null)
        {
            Debug.LogError("[DeploySceneSetup] TutorialRigRoot not found!");
            return;
        }
        ...
        DeployTutorialManager manager = SetupTutorialController(player, camCtrl, ghostSkin);

        // 5. Setup Canvas UI
        SetupCanvasUI(manager);

        // 6. Populate default steps
        ...
        Debug.Log("[DeploySceneSetup] Deploy For Flight scene successfully setup and saved!");
    }

    [MenuItem("Tools/Wire Deploy For Flight Button")]
    public static void ExecuteWireDeployButton()
    {
        if (Application.isBatchMode || EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())
        {
        ...
        if (canvasGo == null)
        {
            Debug.LogError("[DeploySceneSetup] Canvas not found in TutorialSelectScreen!");
            return;
        }
        ...
        foreach (var btn in buttons)
        {
            ...
            if (btnText.Contains("DEPLOY") && btnText.Contains("FLIGHT"))
            {
                ...
                wiredCount++;
                Debug.Log("[DeploySceneSetup] Wired 'DEPLOY FOR FLIGHT' button!");
            }
        }

        EditorSceneManager.MarkSceneDirty(SceneManager.GetActiveScene());
        EditorSceneManager.SaveOpenScenes();
        Debug.Log($"[DeploySceneSetup] Wired {wiredCount} buttons in TutorialSelectScreen and saved!");
    }
    ...
    private static DeployTutorialManager SetupTutorialController(TutorialPlayer player, ModelCameraController camCtrl, TutorialGhostSkin ghostSkin)
    {
        ...
        DeployTutorialManager manager = ctrlGo.AddComponent<DeployTutorialManager>();
    ...
    private static void SetupCanvasUI(DeployTutorialManager manager)
    {
        ...
        oldCanvas = GameObject.Find("DeployTutorialCanvas");
        if (oldCanvas != null)
        {
            Undo.DestroyObjectImmediate(oldCanvas);
        }
        ...
        // Canvas
        GameObject canvasGo = new GameObject("DeployTutorialCanvas", typeof(RectTransform), typeof(Canvas), typeof(CanvasScaler), typeof(GraphicRaycaster));
        ...
        // Step Counter Text
        TextMeshProUGUI stepCountText = CreateText("StepCounterText", topPanel.transform, "STEP 1 / 27", font, 20, TextAlignmentOptions.Center);
        ...
        // Title Text
        TextMeshProUGUI titleText = CreateText("TitleText", topPanel.transform, "Deploy one landing gear", font, 28, TextAlignmentOptions.Center);
        ...
        // Instruction Text
        TextMeshProUGUI instText = CreateText("InstructionText", topPanel.transform, "Use the THUMB to push the button on each leg of the landing gear AND grab the leg AT THE SAME TIME to swing it outwards.", font, 19, TextAlignmentOptions.Center);
